...
import { SocialLink, SkillGroup, ExperienceItem, ProjectItem, EducationItem, StatItem } from './types';
...
export const PERSONAL_INFO = {
  ...
  about: [
    "　　擁有半導體先進製程整合與現代全端軟體架構的雙重專業背景。 於 台積電 及 世界先進 擔任 製程整合工程師 期間，深刻體會到自動化系統為團隊節省大量人力與時間成本的價值，進而跨領域轉職投入軟體開發。",
    "　　現任 台灣大哥大 資深工程師，負責網站系統需求分析、架構設計、程式開發、測試及維運工作。"
  ],
...
export const PROJECT_LINKS: Record<string, { url: string; logo: string }> = {
  "大哥付隨帳收": { url: "https://dcb.oppay.tw/", logo: "/logos/dcb-favicon.ico" },
  "AI 聽寫大哥": { url: "https://www.twmsolution.com/ebgp/ai_votexai", logo: "/logos/ai-votex.png" },
};
...
export const EXPERIENCE_DATA: ExperienceItem[] = [
  {
    ...
    description: [
      "【大哥付隨帳收】OTT 申辦、停車代收、點燈祈福整合平台，串接多套帳務系統並提供外部廠商 API。",
      "建置 OpenSearch API Latency Dashboards，並追蹤各節點 API、DB 呼叫耗時，加速異常排查效率。",
      "導入 SDD (Spec-Driven Development) 開發流程及前後端自動化測試 (Playwright, Selenium, JUnit)。",
      "導入 Design Patterns 重構專案 API，加快後續服務接入速度並降低邏輯錯誤。",
      "建立排程監控系統，於 DB、Log 符合設定條件時自動發送 email 或簡訊通知。",
      "【AI 聽寫大哥】提供多國語音轉文字之記錄平台。",
      "開發通知中心推播功能 API，因應 Web／App 多裝置情境設計全裝置推播及已讀機制。"
    ],
  ...
  },
  {
    ...
    description: [
      "開發微影製程異常區域偵測判定系統（photolithography non-correctable error ink-out system），減少 ~1/3 潛在可靠性失效風險（<0.2% yield loss），並成功被他廠導入使用。",
      "使用 SAS EG 自動化每日撈取資料及繪製圖表作業（systematized SPC chart & auto report），減少約 95% 人力時間成本，並透過標準化提高圖表品質。"
    ],
  ...
  },
  {
    ...
    description: [
      "建置 NTO (New Tape-Out) 製程流程與 inline／WAT 量測程式。",
      "WAT 與良率 (Yield) excursion 問題排除，確保晶圓出貨品質與可靠性指標達標。"
    ],
